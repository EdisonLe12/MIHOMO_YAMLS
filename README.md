...
1. **保留 `China-IP,no-resolve`（规则默认 / 隐私与防止DNS泄漏优先）**：
   ...
   * **潜在分流影响**：判定完全依赖预设域名库；未收录的小众大陆域名将无法触发国内 IP 判定，可能会误判分流到漏网之鱼（走代理），导致国内小网站打不开或提示地区错误，且其 UDP 443 会被拦截并回退至 TCP 传输。

2. **移除 `no-resolve`（可选配置 / 分流精度优先）**：
   * **精准识别机制**：域名未命中规则库时将触发实时 DNS 解析，通过返回的真实 IP 兜底比对，确保所有国内冷门站点均能精准识别走直连 (DIRECT)，避免国内域名或 IP 误走代理，并放行 QUIC 直连加速。
