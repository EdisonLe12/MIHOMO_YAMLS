...
1. **保留 `China-IP,no-resolve`（规则默认）**：
   * **DNS**：规则匹配时不触发递归解析，**规则匹配时不触发本地 DNS 递归反查，境外域名完全交由远端代理服务器进行安全解析，防止本地 DNS 查询泄露、流量审计与域名污染风险，纯本地匹配消除首次建连时延。
   * **潜在分流影响**：判定完全依赖预设域名库；未收录的小众大陆域名将无法触发国内 IP 判定，**可能直接穿透至末尾兜底规则误走代理**，且其 UDP 443 会被拦截并回退至 TCP 传输。

2.**移除 `,no-resolve`（可选配置 / 分流精度优先）**：
   * **精准识别机制**：域名未命中规则库时将触发实时 DNS 解析，**通过返回的真实 IP 兜底比对**，确保所有国内冷门站点均能精准识别走直连（DIRECT），避免国内域名或IP误走代理，并放行 QUIC 直连加速。
